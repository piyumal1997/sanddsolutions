...
const PanelBrandsManagement = () => {
  ...
  const loadBrands = async () => {
    setLoading(true);
    try {
      const res = await protectedFetch(`${import.meta.env.VITE_API_BASE_URL}/api/panel-brands`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editing ? `/api/panel-brands/${editing.id}` : '/api/panel-brands';
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await protectedFetch(`${import.meta.env.VITE_API_BASE_URL}/api/panel-brands/${id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="p-8">
      <h1 className="text-4xl font-bold mb-10 text-gray-900">Panel Brands Management</h1>
