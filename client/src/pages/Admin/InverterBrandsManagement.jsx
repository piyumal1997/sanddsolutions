...
const InverterBrandsManagement = () => {
  ...
  const loadBrands = async () => {
    ...
    try {
      const res = await protectedFetch(`${API_BASE}/api/inverter-brands`);
      if (!res.ok) {
      ...
      console.error('Error loading inverter brands:', err);
      setError('Failed to load inverter brands. Please try again.');
      Swal.fire({
        icon: 'error',
        title: 'Load Error',
        text: err.message || 'Could not load inverter brands.',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editing ? `${API_BASE}/api/inverter-brands/${editing.id}` : `${API_BASE}/api/inverter-brands`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await protectedFetch(`${API_BASE}/api/inverter-brands/${id}`, {
        method: 'DELETE',
      });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl text-gray-700">Loading inverter brands...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 lg:p-10">
      <h1 className="text-4xl lg:text-5xl font-bold mb-10 text-gray-900">Inverter Brands Management</h1>
      ...
      <div className="bg-white p-8 lg:p-10 rounded-2xl shadow-xl mb-12">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              placeholder="e.g. Huawei, Growatt, SMA"
              value={form.name}
      ...
      {brands.length === 0 ? (
        <div className="bg-white p-10 rounded-2xl shadow text-center text-gray-600">
          <p className="text-xl">No inverter brands added yet.</p>
          <p className="mt-2">Add your first inverter brand using the form above.</p>
        </div>
      ) : (
