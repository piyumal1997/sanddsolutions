// server/src/routes/panel-capacities.js
...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to load panel capacities',
      error: err.message,
    });
  }
});
  ...
  try {
    ...
    if (existing.length > 0) {
      return res.status(409).json({
        success: false,
        message: 'Panel capacity with this wattage already exists',
      });
    }
    ...
    res.status(500).json({
      success: false,
      message: 'Failed to create panel capacity',
      error: err.message,
    });
  }
});
  ...
  try {
    ...
    const [result] = await pool.query(
      `UPDATE panel_capacities 
       SET wattage = ?, description = ?, updated_at = NOW()
       WHERE id = ?`,
      [wattage, description?.trim() || null, id]
    );

    if (result.affectedRows === 0) {
      return res.status(404).json({
        success: false,
        message: 'Panel capacity not found',
      });
    }

    res.json({
      success: true,
      message: 'Panel capacity updated successfully',
    });
  } catch (err) {
    console.error('PUT /panel-capacities error:', err.message);
    res.status(500).json({
      success: false,
      message: 'Failed to update panel capacity',
      error: err.message,
    });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      'DELETE FROM panel_capacities WHERE id = ?',
      [id]
    );
    ...
    res.json({
      success: true,
      message: 'Panel capacity deleted successfully',
    });
  } catch (err) {
    console.error('DELETE /panel-capacities error:', err.message);
    res.status(500).json({
      success: false,
      message: 'Failed to delete panel capacity',
      error: err.message,
    });
  }
});
